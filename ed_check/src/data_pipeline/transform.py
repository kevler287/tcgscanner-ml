...
def run_crop_transform(src_dir: Path, dest_dir: Path) -> int:
    train_path_0 = dest_dir / "train" / "other_ed"
    train_path_1 = dest_dir / "train" / "first_ed"
    val_path_0 = dest_dir / "val" / "other_ed"
    val_path_1 = dest_dir / "val" / "first_ed"
    ...
    for subdir in sorted(p for p in src_dir.iterdir() if p.is_dir()):
